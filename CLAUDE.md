...
- 0019 `site_lecteur` APPLIQUEE le 2026-07-18, bascule du site TERMINEE le meme jour (JWT en service, echeance 2036-07-18 ; service_role sortie de `site/.env.local` ; preuve zero fuite + E2E verts). VAGUE 1 TERMINEE ET VERIFIEE le 2026-07-18 : migrations 0019-0023, UI Atelier, consommation site, favicon commitee, circuit chef → site exerce de bout en bout par Arnaud (E2E sans anomalie, zero fuite) — details : `docs/site/CONFORMITE_VAGUE_1.md`. Famille « Paniers Frais » volontairement en attente (future categorie boutique). Prochaine etape : push + deploiement Vercel du site (session dediee — projet distinct, Root Directory `site/`, env SUPABASE_URL + SUPABASE_ANON_KEY + SUPABASE_SITE_LECTEUR_JWT, Ignored Build Step croise avec le projet Atelier ; merge main = prod UNIQUEMENT sur feu vert explicite). Avant deploiement : favicon a choisir.
