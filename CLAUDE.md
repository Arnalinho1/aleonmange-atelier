...
- 0019 `site_lecteur` APPLIQUEE le 2026-07-18, bascule du site TERMINEE le meme jour (JWT en service, echeance 2036-07-18 ; service_role sortie de `site/.env.local` ; preuve zero fuite + E2E verts). PLAN REFERENTIEL TERMINE : 0019 a 0023 appliquees le 2026-07-18 (0024 creneau_retrait reportee au STOP Vague 2). Chantiers restants, a cadrer chacun en nouvelle session : 1) UI Atelier (Catalogue : description + visible_site ; Reglages : emplacements enrichis, horaires boutique, familles de carte) ; 2) consommation cote site des nouvelles colonnes ; 3) favicon ; 4) push + deploiement Vercel du site (projet distinct, Root Directory `site/`, Ignored Build Step sur le projet Atelier). Avant deploiement : favicon a choisir.
