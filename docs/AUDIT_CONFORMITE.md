...
## stock — Stocks & traçabilité (MOCKUP §3.10)
- ✅ 4 KPI foncés, table composants (pastille cat, chevron+nom+cat/seuil/lots, stock + statut badge, boutons Ajuster/Seuil), expand → encart FEFO + mini-table lots (n°/reçu/qté/DLC), colonne « Lots en cours · DLC » triée.
- ✅ CTA réels : réception (lot + mouvement +), ajustement d'inventaire (écart SIGNÉ calculé côté serveur), seuil (upsert). Stock = Σ mouvements signés.
- 🟡 Statut Bas/Rupture = comparaison au seuil UTILISATEUR ; les alertes automatiques (notifications) = point ouvert #2. KPI « Prochaine DLC » sans règle de « DLC proche » figée.
- 🟡 Alerte gaspillage et barres de stock de la maquette omises (gaspi = démo ; barres sans capacité max définie).
- 🟡 Sortie automatique du stock à la vente non branchée (déduction ventes→stock à cadrer : unités de vente ≠ kg).
- CTA : réception ✅ · ajustement ✅ · seuil ✅ (testés en base).
...
- ✅ « Prévision — demain » (carte foncée) : moyenne 7 j des ventes remises DÉPLIÉES par composant × buffer +10 %, badge « Aide à la décision », colonnes maquette, prisme ChanFilter ; plan du jour (barres lots vs suggéré) ; « Enregistrer un lot » RÉEL (lot + entrée stock) ; lots du jour.
...
- 🟡 « Signal communauté » (votes) omis — référentiel social, point ouvert #4. « Appliquer au plan » omis (le plan SE CONSTRUIT en enregistrant les lots). T° à cœur/temps/chefs du drawer maquette omis (mesure MO à cadrer).
- 🟡 Règle +10 % INDICATIVE affichée comme telle — point ouvert #2.
- CTA : enregistrer lot ✅ (testé : lot + mouvement en base).
