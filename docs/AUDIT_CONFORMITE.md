...
## productivity — Productivité (design CD V2 du 12/07/2026, porté le 12/07/2026)
- ✅ **Portage 1:1 de la maquette** `ALM - Atelier FINAL V2 (2026-07-12) (2).html` (référence vérifiée par marqueurs — les exports antérieurs contenaient l'ancien écran). Grille CONFORMITE §1-§6 tenue point par point :
- ✅ **§1 Vue « Tous » comparative, jamais agrégée** : 3 cartes canal (mesure maîtresse — truck/boutique = pic + débit badge `Débit`, traiteur = cycle moyen badge `Cycle`), barres comparatives UNIQUEMENT sur coût matière (€) et temps (h), bandeau « Cumul · même unité » (ventes · € · h — jamais de score composite), insight rendu DYNAMIQUE (calculé des données réelles, une phrase statique deviendrait fausse).
- ✅ **§2 Honnêteté** : tags Calculé (bleu) / Estimé (teal, déclaratif chef) ; amplitude « (observée) = première → dernière vente, pas les horaires d'ouverture » ; couverture TOUJOURS visible (« couv. 100 % · 0 ligne sans coût ») ; temps en heures de travail (« 84 h 51 »), jamais en € ; pic surligné dans l'histogramme.
- ✅ **§3 États vides** : canal sans ventes, histogramme vide (« voir les imports exclus »), aucun service, cycle vide (« les indicateurs de charge restent affichés »), lignes sans coût/temps comptées et affichées.
- ✅ **§4 Périmètre** : aucune marge (« le CA n'est qu'un dénominateur de couverture — la marge vit dans Finances »), aucun coût MO, aucune conversion temps→€, pas de recréation de Ventes & tendances, gaspillage absent.
- ✅ **§5 Anticipation MO** : note exacte « convertibles en coût plus tard, aucun taux appliqué aujourd'hui » sur le bloc temps.
- ✅ **§6 Adaptation par canal** : truck = rythme (pic) + services observés + charge + FILTRE EMPLACEMENT ; boutique = idem en débit pur (pas de sous-vue cycle — arbitrage C&C acté) ; traiteur = 4 KPIs (dont attente de retrait) + table des cycles nommés (client) + charge « en second plan ».
- ✅ **Dates post-facturation** : cycle = `commande_le` → remise (le « saisie » de la maquette EST la prise de commande) ; rythme/heure de service = `occurred_at` (= livre_le). Contrôle de correction validé au centime : truck 764,93 € / 47 h 00 · boutique 1 935,36 € / 84 h 51 · traiteur 585,85 € / 97 h 57 · cumul 732 ventes / 3 286,14 € / 229 h 48 · cycle ≈ 4 j · pics 12 h / 13 h.
- 🟡 **Écarts documentés et validés** : tag « Démo — 7/30 j (projections) » NON repris (artefact de la maquette statique — ici les périodes sont de VRAIS filtres, étiqueter « Démo » du réel serait une fausse honnêteté) ; insight du Cumul dynamique au lieu de statique ; noms de cycle = client de la vente (« Sans client » à défaut).
- CTA : filtres canal / emplacement / période ✅ (lecture seule).
