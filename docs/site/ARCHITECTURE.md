...
- **Correctifs (2026-07-21, EN PRODUCTION)** : (a) ORDRE - le consent update granted doit etre dans `dataLayer` AVANT `gtag('config')`, sinon le premier page_view part en denied (`e73ef5f`) ; (b) EXECUTION - `gtag.js` charge par un `<script>` DOM NATIF (`document.head.appendChild`), PAS par `@next/third-parties`/`next/script` : sur un montage CONDITIONNEL post-hydratation (apres consentement), next/script emet le preload (le fetch) mais n'injecte pas de facon fiable le `<script>` executant -> gtag.js fetche sans s'executer, `window.google_tag_manager` undefined. `@next/third-parties` retire ; `trackEvent` -> `window.gtag('event', nom, params)` (`c402b1a`).
- **Lecons de code (GA / gtag)** : (1) le shim gtag doit etre une `function` CLASSIQUE, JAMAIS une flechee (`arguments` n'existe pas dans une flechee) ; (2) prouver l'etat/l'ordre de `window.dataLayer` par `JSON.stringify(dataLayer.map(a => Array.from(a)))` ; (3) pour tout montage conditionnel, charger gtag.js par `<script>` DOM natif (execution garantie par le contrat HTML, contrairement au chemin decouple render/commit de next/script).
- **Lecon de debug** : un symptome GA identique sur PLUSIEURS appareils du MEME proprietaire n'implique PAS « le site ». Verifier D'ABORD `nslookup googletagmanager.com` sur l'appareil de test (`0.0.0.0` / NXDOMAIN = filtre DNS local : Pi-hole, NextDNS, routeur, VPN, adblock DNS) et retester depuis un reseau VRAIMENT tiers AVANT de toucher au code. Cas reel : filtre DNS sur le Mac + tel du proprietaire, pas le site (des heures de debug code evitables par un `nslookup`).
