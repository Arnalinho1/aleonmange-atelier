...
- **Confirmation e-mail ON** (`mailer_autoconfirm=false`). **Prerequis config (a la main d'Arnaud)** : autoriser `http://localhost:3002/compte/auth/callback` (dev) + `https://aleonmange.app/compte/auth/callback` (prod) dans Supabase Auth > URL Configuration > Redirect URLs (template par defaut suffit, flux PKCE via `emailRedirectTo`).
