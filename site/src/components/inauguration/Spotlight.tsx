 ...
 * au scroll et en boucle courte. Cible introuvable ou invisible -> spotlight
 * plein écran, JAMAIS d'erreur.
 *
 * DUPLIQUÉ depuis src/components/guide/Spotlight.tsx (Atelier) — apps isolées
 * du monorepo, pas d'import partagé (même idiome que les modules email).
 * Différences site : garde de visibilité dans resoudreCible (le lien
 * « Mon compte » du header est masqué sous 640 px -> fallback plein écran)
 * et « Passer » posé sous le header sticky du site (~72 px).
 */

export type CibleSpec = {
  /** Sélecteur CSS de la cible ; null = plein écran. */
  sel: string | null;
  /** Ne retient que le candidat dont le texte contient cette chaîne. */
  contains?: string;
  /** Affine vers un descendant du candidat retenu. */
  inner?: string;
};

export function resoudreCible(spec: CibleSpec): HTMLElement | null {
  if (!spec.sel) return null;
  const visibles = Array.from(document.querySelectorAll<HTMLElement>(spec.sel)).filter((c) => {
    const r = c.getBoundingClientRect();
    return r.width > 0 && r.height > 0;
  });
  let el =
    (spec.contains
      ? visibles.find((c) => (c.textContent ?? "").includes(spec.contains as string))
      : visibles[0]) ?? null;
...
export function Spotlight({
  ...
  zIndex?: number;
  onPasser: () => void;
  /** Clic réel sur la cible — observé, jamais intercepté (inutilisé par la cérémonie). */
  onCibleClick?: () => void;
  /** Signale si la cible existe dans le DOM (inutilisé par la cérémonie). */
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex, pointerEvents: "none" }}>
      ...
      <button
        ...
        onClick={onPasser}
        style={{
          // 86px : sous le header sticky du site (~72 px) — la maquette posait
          // 16px mais spotlighte justement le header (ch2 « Mon compte »).
          position: "fixed",
          top: 86,
          right: 16,
          pointerEvents: "auto",
