...
export type EmplacementTruck = {
  ...
  jour: string; // « Mardi »
  horaire: string; // provisoire : amplitude de service par defaut
...
export async function emplacementsTruck(): Promise<EmplacementTruck[]> {
  ...
  const { data, error } = await supabase
    .from("emplacement")
    .select("id, libelle, jour_semaine")
    .eq("actif", true)
  ...
  return ((data ?? []) as { id: string; libelle: string; jour_semaine: number | null }[]).map((e) => ({
    id: e.id,
    nom: e.libelle,
    jour: e.jour_semaine != null ? JOURS[e.jour_semaine] ?? "" : "",
    horaire: HORAIRE_SERVICE_TRUCK,
